...
from langgraph_expert import langgraph_coder, LangGraphDeps, list_documentation_pages_helper
...
reasoner = Agent(  
    ...
    system_prompt='You are an expert at coding AI agents with LangGraph and defining the scope for doing so.',  
)
...
async def coder_agent(state: AgentState, writer):    
    ...
    # Get the message history into the format for Langgraph
    ...
    if is_ollama:
        ...
        result = await langgraph_coder.run(state['latest_user_message'], deps=deps, message_history= message_history)
        writer(result.data)
    else:
        async with langgraph_coder.run_stream(
